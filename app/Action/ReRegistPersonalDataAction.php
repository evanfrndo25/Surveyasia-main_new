...
class ReRegistPersonalDataAction
{
    public function execute(PersonalDataRequest $request)
    {
        ...
        if ($request->File('image_ktp')) {
            ...
            //Delete old data
            $oldDataImage = UsersProfile::where('user_id', $request->user()->id)->value('image_ktp');
            Storage::delete($oldDataImage);
        }
        // save database
        ...
        return UsersProfile::where('user_id', $request->user()->id)
            ->update($profile);
    }
}
